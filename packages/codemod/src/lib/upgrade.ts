...
function runCodemods(
  ...
  if (notImplementedAvailable) {
    log(
      `Some ${versionLabel} codemods require manual changes. Please search your codebase for \`FIXME(@omni-stack-upgrade-v5): \` comments and follow the instructions to complete the upgrade.`,
    );
  }
...
export function upgrade(options: TransformOptions) {
  ...
  if (notImplementedAvailable) {
    log(
      'Some codemods require manual changes. Please search your codebase for `FIXME(@omni-stack-upgrade-v5): ` comments and follow the instructions to complete the upgrade.',
    );
  }
