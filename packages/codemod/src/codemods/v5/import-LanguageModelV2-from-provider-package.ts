...
/**
 * Codemod to update imports of LanguageModelV2 and related types from '@omni-stack/core' to '@omni-stack/provider'.
 * @see https://ai-sdk.dev/docs/migration-guides/migration-guide-5-0#language-model-v2-import
 */
export default createTransformer((fileInfo, api, options, context) => {
  const { j, root } = context;

  // Find all import declarations from '@omni-stack/core'
  root.find(j.ImportDeclaration).forEach(importPath => {
    ...
    // Find specifiers that should be moved to '@omni-stack/provider'
    ...
    if (remainingSpecifiers.length === 0) {
      ...
      node.source.value = '@omni-stack/provider';
      context.messages.push(`Updated import from '@omni-stack/core' to '@omni-stack/provider'`);
    } else {
      ...
      context.messages.push(
        `Split import: moved some imports from '@omni-stack/core' to '@omni-stack/provider'`,
      );
    }
  });
});
