...
The NVIDIA NIM provider is available via the `@omni-stack/openai-compatible` module as it is compatible with the OpenAI API.
...
To use NVIDIA NIM, you can create a custom provider instance with the `createOpenAICompatible` function from `@omni-stack/openai-compatible`:
