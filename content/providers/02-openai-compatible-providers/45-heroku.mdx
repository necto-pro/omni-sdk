...
The Heroku provider is available via the `@open-stack/openai-compatible` module as it is compatible with the OpenAI API.
...
To use Heroku, you can create a custom provider instance with the `createOpenAICompatible` function from `@open-stack/openai-compatible`:
