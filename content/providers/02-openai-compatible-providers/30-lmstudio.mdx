...
The LM Studio provider is available via the `@omni-stack/openai-compatible` module as it is compatible with the OpenAI API.
...
To use LM Studio, you can create a custom provider instance with the `createOpenAICompatible` function from `@omni-stack/openai-compatible`:
