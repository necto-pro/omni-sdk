...
The `@open-stack-tool/parser` package offers three middleware variants:
...
```ts
import { wrapLanguageModel } from 'ai';
import { gemmaToolMiddleware } from '@open-stack-tool/parser';
...
```ts
import type { LanguageModelV3Middleware } from '@open-stack/provider';
