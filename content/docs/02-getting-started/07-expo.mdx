...
Install `ai`, `@omni-stack/react` and `@omni-stack/openai`, the AI package, the AI React package and AI SDK's [ OpenAI provider ](/providers/ai-sdk-providers/openai) respectively.
...
<div className="my-4">
  <Tabs items={['pnpm', 'npm', 'yarn', 'bun']}>
    <Tab>
      <Snippet text="pnpm add ai @omni-stack/openai @omni-stack/react zod" dark />
    </Tab>
    <Tab>
      <Snippet text="npm install ai @omni-stack/openai @omni-stack/react zod" dark />
    </Tab>
    <Tab>
      <Snippet text="yarn add ai @omni-stack/openai @omni-stack/react zod" dark />
    </Tab>
    <Tab>
      <Snippet text="bun add ai @omni-stack/openai @omni-stack/react zod" dark />
    </Tab>
  </Tabs>
</div>
...
import { streamText, UIMessage, convertToModelMessages } from '@omni-stack/core';
...
2. Call [`streamText`](/docs/reference/ai-sdk-core/stream-text), which is imported from the `ai` package. This function accepts a configuration object that contains a `model` provider (imported from `@omni-stack/openai`) and `messages` (defined in step 1). You can pass additional [settings](/docs/ai-sdk-core/settings) to further customise the model's behaviour.
...
import { openai } from '@omni-stack/openai';
import { streamText, UIMessage, convertToModelMessages, tool } from '@omni-stack/core';
import { z } from 'zod';
