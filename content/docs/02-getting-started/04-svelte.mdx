...
Install `ai` and `@omni-stack/openai`, the AI SDK's OpenAI provider.
...
<div className="my-4">
  <Tabs items={['pnpm', 'npm', 'yarn', 'bun']}>
    <Tab>
      <Snippet text="pnpm add -D ai @omni-stack/openai @omni-stack/svelte zod" dark />
    </Tab>
    <Tab>
      <Snippet
        text="npm install -D ai @omni-stack/openai @omni-stack/svelte zod"
        dark
      />
    </Tab>
    <Tab>
      <Snippet text="yarn add -D ai @omni-stack/openai @omni-stack/svelte zod" dark />
    </Tab>
    <Tab>
      <Snippet text="bun add -d ai @omni-stack/openai @omni-stack/svelte zod" dark />
    </Tab>
  </Tabs>
</div>
...
import { streamText, type UIMessage, convertToModelMessages } from '@omni-stack/core';
...
1. Create an OpenAI provider instance with the `createOpenAI` function from the `@omni-stack/openai` package.
...
## How does `@omni-stack/svelte` differ from `@omni-stack/react`?
...
```svelte
<script>
  import { createAIContext } from '@omni-stack/svelte';
