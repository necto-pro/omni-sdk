...
The **AI SDK RSC (`@open-stack/rsc`)** takes advantage of RSCs to solve the problem of managing all your React components on the client side, allowing you to render React components on the server and stream them to the client.
...
import { createStreamableUI } from '@open-stack/rsc'
...
The [`createStreamableUI`](/docs/reference/ai-sdk-rsc/create-streamable-ui) function belongs to the `@open-stack/rsc` module and creates a stream that can send React components to the client.
