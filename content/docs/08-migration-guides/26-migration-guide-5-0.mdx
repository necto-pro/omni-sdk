...
- `ai` package: `5.0.0`
- `@open-stack/provider` package: `2.0.0`
- `@open-stack/provider-utils` package: `3.0.0`
- `@open-stack/*` packages: `2.0.0` (other `@open-stack` packages)
...
```
npm install ai @open-stack/react @open-stack/openai zod@^4.1.8
```
...
```sh
npx @open-stack/codemod v5/rename-format-stream-part src/
```

See also the [table of codemods](#codemod-table). In addition, the latest set of
codemods can be found in the
[`@open-stack/codemod`](https://github.com/vercel/ai/tree/main/packages/codemod/src/codemods)
repository.
...
### `@open-stack/rsc` Package Extraction
...
<Note>Don't forget to install the new package: `npm install @open-stack/rsc`</Note>

### React UI Hooks Moved to `@open-stack/react`

The deprecated `ai/react` export has been removed in favor of `@open-stack/react`.
...
<Note>
  Don't forget to install the new package: `npm install @open-stack/react`
</Note>
...
```tsx filename="AI SDK 4.0"
import { useChat, type Message } from '@open-stack/react';
...
```tsx filename="AI SDK 5.0"
import { useChat, type UIMessage } from '@open-stack/react';
...
```typescript filename="@open-stack/vue v1"
<script setup>
...
```typescript filename="@open-stack/vue v2"
<script setup>
...
```js filename="@open-stack/svelte v1"
...
```js filename="@open-stack/svelte v2"
import { Chat } from '@open-stack/svelte';
import { DefaultChatTransport } from 'ai';
...
#### `@open-stack/ui-utils` Package Removal

The `@open-stack/ui-utils` package has been removed and its exports moved to the main `ai` package.

```tsx filename="AI SDK 4.0"
import { getTextFromDataUrl } from '@open-stack/ui-utils';
```
...
```tsx filename="AI SDK 4.0"
import { useAssistant } from '@open-stack/react';
```
...
#### LangChain Adapter Moved to `@open-stack/langchain`

The `LangChainAdapter` has been moved to `@open-stack/langchain` and the API has been updated to use UI message streams.
...
<Note>
  Don't forget to install the new package: `npm install @open-stack/langchain`
</Note>

#### LlamaIndex Adapter Moved to `@open-stack/llamaindex`

The `LlamaIndexAdapter` has been extracted to a separate package `@open-stack/llamaindex` and follows the same UI message stream pattern.
...
```tsx filename="AI SDK 5.0"
import { toUIMessageStream } from '@open-stack/llamaindex';
import { createUIMessageStreamResponse } from 'ai';
...
<Note>
  Don't forget to install the new package: `npm install @open-stack/llamaindex`
</Note>
...
`LanguageModelV3` must now be imported from `@open-stack/provider`.
...
import { openai, type OpenAIResponsesProviderOptions } from '@open-stack/openai';
