...
- `ai` package: `5.0.0`
- `@omni-stack/provider` package: `2.0.0`
- `@omni-stack/provider-utils` package: `3.0.0`
- `@omni-stack/*` packages: `2.0.0` (other `@omni-stack` packages)
...
```
npm install ai @omni-stack/react @omni-stack/openai zod@^4.1.8
```
...
```sh
npx @omni-stack/codemod v5/rename-format-stream-part src/
```

See also the [table of codemods](#codemod-table). In addition, the latest set of
codemods can be found in the
[`@omni-stack/codemod`](https://github.com/vercel/ai/tree/main/packages/codemod/src/codemods)
repository.
...
```tsx filename="AI SDK 4.0"
import { CoreMessage } from '@omni-stack/core';
```
...
```tsx filename="AI SDK 4.0"
import { Message, CreateMessage } from '@omni-stack/core';
```

```tsx filename="AI SDK 5.0"
import { UIMessage, CreateUIMessage } from '@omni-stack/core';
```
...
```tsx filename="AI SDK 4.0"
import { type Message } from '@omni-stack/core'; // v4 Message type
...
```tsx filename="AI SDK 5.0"
import { type UIMessage, type ModelMessage } from '@omni-stack/core';
...
```tsx filename="AI SDK 5.0"
import { createUIMessageStream, createUIMessageStreamResponse } from '@omni-stack/core';
...
```tsx filename="AI SDK 4.0"
import { openai } from '@omni-stack/openai';
import { createDataStreamResponse, streamText } from '@omni-stack/core';
...
```tsx filename="AI SDK 4.0"
import { ToolExecutionError } from '@omni-stack/core';
...
```tsx filename="AI SDK 5.0"
import { dynamicTool } from '@omni-stack/core';
import { z } from 'zod';
...
```tsx filename="AI SDK 5.0"
import { MCPClient } from '@omni-stack/core';
...
```tsx filename="AI SDK 5.0"
import { isToolUIPart, getToolName } from '@omni-stack/core';
...
```tsx filename="AI SDK 5.0"
import { stepCountIs, hasToolCall } from '@omni-stack/core';
...
### `@omni-stack/rsc` Package Extraction
...
<Note>Don't forget to install the new package: `npm install @omni-stack/rsc`</Note>

### React UI Hooks Moved to `@omni-stack/react`

The deprecated `ai/react` export has been removed in favor of `@omni-stack/react`.
...
<Note>
  Don't forget to install the new package: `npm install @omni-stack/react`
</Note>
...
import { streamText, convertToModelMessages, stepCountIs } from '@omni-stack/core';
import { openai } from '@omni-stack/openai';
...
```tsx filename="AI SDK 4.0"
import { useChat, type Message } from '@omni-stack/react';
...
```tsx filename="AI SDK 5.0"
import { useChat, type UIMessage } from '@omni-stack/react';
...
```tsx filename="AI SDK 4.0"
import type { RequestOptions } from '@omni-stack/core';
```

```tsx filename="AI SDK 5.0"
import type { CompletionRequestOptions } from '@omni-stack/core';
```
...
```typescript filename="@omni-stack/vue v1"
<script setup>
...
```typescript filename="@omni-stack/vue v2"
<script setup>
...
```js filename="@omni-stack/svelte v1"
...
```js filename="@omni-stack/svelte v2"
import { Chat } from '@omni-stack/svelte';
import { DefaultChatTransport } from '@omni-stack/core';
...
#### `@omni-stack/ui-utils` Package Removal

The `@omni-stack/ui-utils` package has been removed and its exports moved to the main `ai` package.

```tsx filename="AI SDK 4.0"
import { getTextFromDataUrl } from '@omni-stack/ui-utils';
```

```tsx filename="AI SDK 5.0"
import { getTextFromDataUrl } from '@omni-stack/core';
```
...
```tsx filename="AI SDK 4.0"
import { useAssistant } from '@omni-stack/react';
```
...
#### LangChain Adapter Moved to `@omni-stack/langchain`

The `LangChainAdapter` has been moved to `@omni-stack/langchain` and the API has been updated to use UI message streams.
...
<Note>
  Don't forget to install the new package: `npm install @omni-stack/langchain`
</Note>

#### LlamaIndex Adapter Moved to `@omni-stack/llamaindex`

The `LlamaIndexAdapter` has been extracted to a separate package `@omni-stack/llamaindex` and follows the same UI message stream pattern.

```tsx filename="AI SDK 4.0"
import { LlamaIndexAdapter } from '@omni-stack/core';

const response = LlamaIndexAdapter.toDataStreamResponse(stream);
```

```tsx filename="AI SDK 5.0"
import { toUIMessageStream } from '@omni-stack/llamaindex';
import { createUIMessageStreamResponse } from '@omni-stack/core';
...
<Note>
  Don't forget to install the new package: `npm install @omni-stack/llamaindex`
</Note>
...
```tsx filename="AI SDK 4.0"
import { createDataStream, formatDataStreamPart } from '@omni-stack/core';
...
```tsx filename="AI SDK 5.0"
import { createUIMessageStream } from '@omni-stack/core';
...
```tsx filename="AI SDK 4.0"
import { DataStreamToSSETransformStream } from '@omni-stack/core';
```

```tsx filename="AI SDK 5.0"
import { JsonToSseTransformStream } from '@omni-stack/core';
```
...
`LanguageModelV3` must now be imported from `@omni-stack/provider`.

```tsx filename="AI SDK 4.0"
import { LanguageModelV3 } from '@omni-stack/core';
```
...
```tsx filename="AI SDK 4.0"
import { experimental_wrapLanguageModel } from '@omni-stack/core';
```
...
```tsx filename="AI SDK 5.0"
import { streamText, convertToModelMessages, UIMessage } from '@omni-stack/core';
import { openai } from '@omni-stack/openai';
...
import { openai, type OpenAIResponsesProviderOptions } from '@omni-stack/openai';
...
```tsx filename="AI SDK 5.0"
import { simulateStreamingMiddleware, wrapLanguageModel } from '@omni-stack/core';
