...
- `ai-sdk/*` packages: `1.0.*` (other `@open-stack` packages)
...
```sh
npx @open-stack/codemod v4/replace-baseurl src/
```
...
import { render } from '@open-stack/rsc';
```

```ts filename="AI SDK 4.0"
import { streamUI } from '@open-stack/rsc';
```
...
You need to install the `@open-stack/svelte`, `@open-stack/vue`, and `@open-stack/solid` packages directly.
...
```ts filename="AI SDK 3.4"
import { experimental_useAssistant } from '@open-stack/react';
```
