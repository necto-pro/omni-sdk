...
import { experimental_createMCPClient, streamText } from '@omni-stack/mcp';
...
Let's create a simple React component that imports the `useCompletion` hook from the `@omni-stack/react` module. The `useCompletion` hook will call the `/api/completion` endpoint when a button is clicked. The endpoint will generate text based on the input prompt and stream it to the client.
